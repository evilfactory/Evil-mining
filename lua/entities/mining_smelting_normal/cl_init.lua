...
function ENT:Think()
    ...
    self:SetNWFloat("porce",self:GetNWFloat("porce")+0.6)
    self:SetNWFloat("combustivel",self:GetNWFloat("combustivel")-(self:Getoremass1()/10+self:Getoremass2()/10)*0.03)
else
...
if SERVER then return end
        ...
        local combusti = self:GetNWFloat("combustivel")
		 ...
		 cam.Start3D2D(self:GetPos()+Vector(0,0,45), Angle(0,self:GetNWInt("rota", 0),90), 0.1)
		 	draw.SimpleText( "Normal Smelting", "FonteEntidade1", 0, -200, Color( 0, 255, 0, 255 ),1, 1 )
