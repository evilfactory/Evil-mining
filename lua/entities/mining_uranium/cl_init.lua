...
if SERVER then return end
    ...
    if ply:GetEyeTrace().Entity == self || ply:GetNWBool("mining_aiminfo") == false then
 		 cam.Start3D2D(self:GetPos()+Vector(0,0,15), Angle(0,self:GetNWInt("rota", 0),90), 0.1)
		    draw.SimpleText( "Uranium "..self:GetNWString("typ"), "FonteEntidade1", 0, -110, Color( 0, 255, 0, 255 ),1, 1 )
		 	draw.SimpleText( math.Round(self:GetNWFloat("Money")).."$", "FonteEntidade1", 0, 0, Color( 0, 255, 0, 255 ),1, 1 )
		 cam.End3D2D()
